...
import game.general.Board;
import game.general.Game;
import game.general.Move;
import game.general.Piece;
import java.awt.*;
...
import game.player.PlayerHuman;
import game.tools.Index;
import java.awt.image.BufferedImage;
import java.io.IOException;
import java.util.logging.Level;
import java.util.logging.Logger;
import javax.imageio.ImageIO;
import speech.recognizer.SpeechToTextConverter;
...
public class Gui extends JPanel implements Runnable, MouseListener {
    ...
    public Gui() {
        ...
        Image img = null;
        URL imgURL = null;
        imgURL = this.getClass().getResource(jarGraphicsPath + "Microphone.png");
        img = toolkit.getImage(imgURL);

        BufferedImage myPicture2;
        try {
            myPicture2 = ImageIO.read(imgURL);

            addButton.setIcon(new ImageIcon(myPicture2));
        } catch (IOException ex) {
            Logger.getLogger(Gui.class.getName()).log(Level.SEVERE, null, ex);
        }

        addButton.setPreferredSize(new Dimension(60, 60));
        addPanel.setPreferredSize(new Dimension(80, 80));
    ...
    /**
     * Responsible for drawing graphics on the screen. Paint method is run every
    ...
    public void drawPieces(Graphics2D g2) {
        ...
        setG1(g2);
        Graphics2D g3 = (Graphics2D) getG1();
        //get all pieces
        ...
        for (Piece p : pieces) {
            ...
            if (p != null) {
                ...
                g3.drawImage(img, idx.getX(), idx.getY(), this);
            }
        }
    }
    ...
    /**
     * Returns Index object which contains x&y coordinates for upper left corner
     * of selected square notation.
     *
     * @param notation
     * @return Index
     */
    ...
    private void startCommand(ActionEvent e) {
        ...
        if (isSrcSelected() && isDestSelected()) {
            ...
            System.err.println(getSelectedSrc() + " " + getSelectedDest());
            movePiece(board, getMove(), player, getSelectedSrc(), getSelectedDest());
            //repaint the board
            repaint();
            //the movement has been done
            ...
            //check if check mate
            if (playerAI.isCheckmate(player1)) {
                JOptionPane.showMessageDialog(f, "Check Mate!", "Chess", JOptionPane.PLAIN_MESSAGE);
            }
            //another player's turn
            if (isHumanVsHumanMode()) {
                if (getGame().getWhosTurn() == 0) {
